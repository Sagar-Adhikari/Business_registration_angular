...
@Component({
  selector: 'app-month-wise-revenue',
  templateUrl: './month-wise-revenue.component.html',
  styleUrls: ['./month-wise-revenue.component.scss'],
  providers: [GraphService]
})
export class MonthWiseRevenueComponent implements OnInit {
  ...
  constructor(private globalService: GlobalService,
    ...
    this.title = `आर्थिक बर्ष ${currentFY} मा ब्यवसायकाे प्रकृति अनुसार मासिक रूपमा दर्ता तथा नबिकरणबाट संकलन भएकाे राजस्व`;

    this.graphService.getMonthWiseProductRevenueWithFYList(currentFY).subscribe(x => {
    ...
    for (let i = 0; i < distinctClass.length; i++) {
      ...
      for (let j = 0; j < monthList.length; j++) {
        ...
        if (monthArray.length > 0) {
          data.push(monthArray[0].taxRevenue);
        } else {
          data.push(0);
        }
      }
  ...
  buttonClick(flag: string) {
    ...
    this.title = `आर्थिक बर्ष ${thisFy} मा ब्यवसायकाे प्रकृति अनुसार मासिक रूपमा दर्ता तथा नबिकरणबाट संकलन भएकाे राजस्व`;
    this.graphService.getMonthWiseRevenue(this.fiscalYearList[this.currentIndex].fiscalYear).subscribe(x => {
      this.loadGraph(x);
      this.globalService.setLoading(false);
    }, error => {
      this.globalService.showErrorMessage(error.message);
